...
- [ ] 初期アカウントを含むusername重複を拒否する。
- [ ] 環境Aでの登録が環境Bのusersを変更しない。
...
- [ ] セッションID更新前の認証状態を再利用できない。
- [ ] ログアウトでは登録データを消さず、サーバー再起動では登録データと認証状態が消える。
...
- [ ] ログイン中と未ログインの状態が区別できる。
- [ ] SQL表示、演習メニュー、版切替などの対象外UIを追加しない。
...
- [ ] username・passwordだけで登録・ログインできる。メールやプロフィールは要求しない。
- [ ] 重複・空値・長さ超過・誤パスワードを拒否する。
- [ ] ログアウトと再ログインで同じDBが使え、旧認証Cookieは無効になる。
- [ ] password・password_digestがHTML、エラー、ログへ出ず、登録・認証処理に意図的なSQLiを入れていない。
